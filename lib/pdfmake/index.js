const PdfPrinter = require('pdfmake')
const path = require('path')

const fonts = {
  Nunito: {
    normal: path.join(__dirname, '.', 'fonts/Nunito/Nunito-Regular.ttf'),
    bold: path.join(__dirname, '.', 'fonts/Nunito/Nunito-Bold.ttf'),
    italics: path.join(__dirname, '.', 'fonts/Nunito/Nunito-RegularItalic.ttf'),
    bolditalics: path.join(__dirname, '.', 'fonts/Nunito/Nunito-BoldItalic.ttf')
  },
  'Nunito Sans': {
    normal: path.join(__dirname, '.', 'fonts/Nunito_Sans/NunitoSans-Regular.ttf'),
    bold: path.join(__dirname, '.', 'fonts/Nunito_Sans/NunitoSans-Bold.ttf'),
    italics: path.join(__dirname, '.', 'fonts/Nunito_Sans/NunitoSans-RegularItalic.ttf'),
    bolditalics: path.join(__dirname, '.', 'fonts/Nunito_Sans/NunitoSans-BoldItalic.ttf')
  },
  Roboto: {
    normal: path.join(__dirname, '.', 'fonts/Roboto-Regular.ttf'),
    bold: path.join(__dirname, '.', 'fonts/Roboto-Medium.ttf'),
    italics: path.join(__dirname, '.', 'fonts/Roboto-Italic.ttf'),
    bolditalics: path.join(__dirname, '.', 'fonts/Roboto-MediumItalic.ttf')
  },
  Courier: {
    normal: 'Courier',
    bold: 'Courier-Bold',
    italics: 'Courier-Oblique',
    bolditalics: 'Courier-BoldOblique'
  },
  Helvetica: {
    normal: 'Helvetica',
    bold: 'Helvetica-Bold',
    italics: 'Helvetica-Oblique',
    bolditalics: 'Helvetica-BoldOblique'
  },
  Times: {
    normal: 'Times-Roman',
    bold: 'Times-Bold',
    italics: 'Times-Italic',
    bolditalics: 'Times-BoldItalic'
  },
  Symbol: {
    normal: 'Symbol'
  },
  ZapfDingbats: {
    normal: 'ZapfDingbats'
  }
}

const printer = new PdfPrinter(fonts)

const getDoc = pdfDoc => {
  const chunks = []
  return new Promise((resolve, reject) => {
    pdfDoc.on('data', chunk => chunks.push(chunk))
    pdfDoc.on('end', () => resolve(Buffer.concat(chunks)))
    pdfDoc.on('error', error => reject(error))
    pdfDoc.end()
  })
}

/**
 *
 * @param {import('pdfmake/interfaces').TDocumentDefinitions} doc
 * @param {*} options
 * @returns {Promise<Buffer>}
 */
const superPdfMake = (doc, options) => {
  if (!doc) {
    throw Error('Missing required input: content')
  }
  if (options && typeof options !== 'object') {
    throw Error('Invalid input: options must be of type object')
  }
  if (options) {
    if (typeof options.type === 'number') {
      options.type = options.type.toString()
    }
    if (!options.type && !options.version) {
      throw Error('Missing required input: options.type and options.version')
    }
    if (typeof options.type !== 'string' || typeof options.version !== 'string') {
      throw Error('Invalid input: options.type must be type string or number and options.version must be of type string')
    }

    const validTypes = ['2']
    const validVersions = ['B']
    if (!validTypes.includes(options.type)) {
      throw Error(`Invalid input: options.type must be one of: ${validTypes.join(', ')}`)
    }
    if (!validVersions.includes(options.version)) {
      throw Error(`Invalid input: options.version must be one of: ${validVersions.join(', ')}`)
    }
    doc.version = '1.7' // Hakke peil på hva forskjell er, annet enn at alt nyere enn default 1.3 er nødvendig for PDF/A
    if (options.type === '2' && options.version === 'B') { // Dritmåte for backwards compatibility
      doc.subset = 'PDF/A-2b'
      doc.tagged = true
    } else {
      throw Error(`Invalid combination of options.type (${options.type}) and options.version (${options.version})`)
    }
  }
  const pdfDoc = printer.createPdfKitDocument(doc)

  return getDoc(pdfDoc)
}

module.exports = { superPdfMake }
