...
function getDocumentDefinition(id, language = defaultLanguage) {
  ...
  try {
    ...
    logger.warn(`document-definitions - Could not find ${definition}. Will try fallback`)
  }
  ...
  try {
    ...
    logger.warn(`document-definitions - Could not find fallback document definition ${fallback} either!`)
